...
class EditableLabel(QLabel):
    ...
    def finish_editing(self):
        """完成编辑模式"""
        if not self.is_editing or not hasattr(self, 'editor'):
            return
    ...
    def eventFilter(self, obj, event):
        """事件过滤器，处理点击其他地方完成编辑"""
        if event.type() == event.FocusOut and hasattr(self, 'editor') and obj == self.editor:
            # 延迟处理，避免立即失去焦点
...
class ComboBoxLabel(QLabel):
    """下拉选择的标签类"""
    valueChanged = pyqtSignal(str, str, str)  # field_name, old_value, new_value

    def __init__(self, text="", field_name="", options=None, parent=None):
        super().__init__(text, parent)
        self.field_name = field_name
    ...
    def show_combo_box(self):
        ...
        index = self.combo.findText(old_value)
        if index >= 0:
        ...
        if layout:
            ...
            for i in range(layout.count()):
                if layout.itemAt(i).widget() == self:
                    layout.takeAt(i)
                    layout.addWidget(self.combo, self.property("row"), self.property("col"))
                    break

        self.combo.setFocus()
        self.combo.showPopup()

        # 连接信号 - 使用activated而不是currentIndexChanged
        self.combo.activated.connect(lambda: self.on_combo_selected(old_value))

    def on_combo_selected(self, old_value):
        """组合框选项被选择"""
        ...
        if layout:
            # 替换组合框为标签
            for i in range(layout.count()):
                if layout.itemAt(i).widget() == self.combo:
                    layout.takeAt(i)
                    layout.addWidget(self, self.property("row"), self.property("col"))
                    break
        ...
        if old_value != new_value:
            self.valueChanged.emit(self.field_name, old_value, new_value)
...
class ImageViewer(QMainWindow):
    ...
    def init_field_labels(self):
        ...
        # 定义字段配置：(字段显示名, JSON字段路径, 字段类型, 选项)
        field_configs = [
            ("图像ID", "model_response.image_id", "text", None),
            ("质量标签", "model_response.quality_tags", "combo", ["清晰", "模糊", "遮挡", "反光", "低光照", "透视畸变", "无干扰"]),
            ("版式类型", "model_response.layout_type", "combo", ["横版新版", "竖版旧版", "个体户版", "外资/港澳版", "特殊行业版", "多页/拼接"]),
            ("年份", "model_response.year", "text", None),
            ("省份", "model_response.province", "text", None),
            ("统一社会信用代码", "model_response.annotations.统一社会信用代码", "text", None),
            ("企业名称", "model_response.annotations.企业名称", "text", None),
            ("法定代表人", "model_response.annotations.法定代表人", "text", None),
            ("注册资本", "model_response.annotations.注册资本", "text", None),
            ("成立日期", "model_response.annotations.成立日期", "text", None),
            ("经营范围", "model_response.annotations.经营范围", "text", None)
        ]

        for i, (display_name, json_path, field_type, options) in enumerate(field_configs):
            row = i
            col = 0
            ...
            # 字段值显示（可直接编辑）
            if field_type == "combo":
                value_label = ComboBoxLabel("暂无数据", display_name, options)
                value_label.valueChanged.connect(self.on_field_value_changed)
            else:
